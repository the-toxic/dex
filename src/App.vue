  ...
  <v-app :class="{'pageReady': isRouterReady}">
    ...
    <v-app-bar app clipped-left :hide-on-scroll="$vuetify.breakpoint.mdAndUp" class="mx-auto mb-8">
      <v-container fluid class="pa-0 fill-height">
        ...
        <v-btn plain :to="{name: 'Home'}" :ripple="false" class="d-none d-md-inline-flex" active-class="activeMenu">Main</v-btn>
    ...
    <v-footer id="mainFooter" absolute bottom padless color="#1E1E1E" class="justify-center fill-width pa-0">
      <div class="smallFooter d-flex justify-space-between align-center flex-wrap fill-width">
        ...
        <v-icon :color="!wsConnected ? 'red' : (wsConnected === 'loading' ? 'orange' : 'green')">{{ wsConnected ? 'mdi-check-network' : 'mdi-close-network' }}</v-icon>
