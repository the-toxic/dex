      ...
      <v-card-text class="d-flex justify-start align-center flex-wrap pa-4">
		...
		>
			<template v-slot:item.pair_name="{ item }">
				<v-btn :to="{name: 'Pair', params: {network: item.raw.network, pairAddr: item.raw.pair_addr}}" rounded variant="text"  class="text-none">
					<span class="text-disabled mr-2">#{{ item.index + 1 }}</span>
					<img v-if="item.raw.token0?.icon" :src="item.raw.token0.icon" :alt="item.raw.token0.symbol" width="24" class="va-top">
					<img v-if="item.raw.token1?.icon" :src="item.raw.token1.icon" :alt="item.raw.token1.symbol" width="24" class="va-top mx-2">
					{{ item.raw.pair_name }}
