...
async function startApp() {
  ...
  if(!accounts.length && store.getters['wallet/account']) {
    await store.dispatch('wallet/logOut', true)
  }
...
//     if(!provider) {
//       await store.dispatch('showAlert', `Please connect your Wallet`)
//       resolve(false); return
//     }
//     let chainId = await provider.send('eth_chainId', []); // await provider.getNetwork().chainId
//
//     if (+chainId !== +networks[chainName].id) {
//       if(logoutOnError) await store.dispatch('wallet/logOut', false)
//       if(provider.connection.url === 'metamask') { // wallet connect
//         await store.dispatch('showAlert', `Please connect your Wallet to ${networks[chainName].name} network`)
//       } else {
//         await store.dispatch('showAlert', `Please switch your Wallet network to ${networks[chainName].name} and re-login on site.`)
//       }
//       resolve(false); return
//     }
//     resolve(true)
...
async function handleAccountChanged(accounts) { // if change account in metamask or remove connect metamask to site
  ...
  if(store.getters['wallet/account']) {
    ...
    store.dispatch('showAlert', {msg: `Account is changed. Please reconnect your wallet.`, color: 'info'}).then()
  }
}
...
export async function requestSign() {
  ...
  if(!accounts.length) {
    await store.dispatch('showAlert', `Please connect Wallet.`)
    return false
  }
  const jwt = store.getters['wallet/jwt']
  if(!jwt) {
    await store.dispatch('showAlert', `Session expired. Try connect Wallet`) // хз нужно ли
    return false
  }
  ...
  try {
    ...
    if(provider.connection.url !== 'metamask') { // wallet-connect
      await store.dispatch('showAlert', {msg: 'To continue, open your wallet`s app and confirm Sign Request', color: 'info'})
    }
    ...
    if(e.code === 4001) { // on close metamask window
      await store.dispatch('showAlert', 'Process is interrupted. Your Wallet is not signed with Cryptotanks.')
      return false
    }
  }
  if(!sign_phrase) {
    await store.dispatch('showAlert', 'Error on sign your Wallet on Cryptotanks.')
    return false
  }

  await store.dispatch('wallet/sign', sign_phrase)
...
export async function connect() {
  ...
  if(accounts.length) { // если уже подключен, дергаем sign
    ...
    await store.dispatch('wallet/login', {
      account: accounts[0],
      // jwt: result.jwt,
    ...
    try {
      ...
      if (e.code === 4001) { // close window connect
        await store.dispatch('showAlert', 'Process is interrupted. Try again connect Wallet.')
      } else if(e.code && e.code === -32002) {
        await store.dispatch('showAlert', 'It is necessary to complete the registration in the Wallet. Click on the extension icon and continue registration.')
      } else if(e.code && e.code === -32603) {
        await store.dispatch('showAlert', 'Wallet internal error.')
      } else {
        await store.dispatch('showAlert', `Undefined error with code ${e.code}. Please contact with support.`)
      }
      return false
    }
